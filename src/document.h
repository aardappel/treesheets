...
struct Document
{
    ...
    struct MyPrintout : wxPrintout
    {
        ...
        bool OnBeginDocument(int startPage, int endPage) { return wxPrintout::OnBeginDocument(startPage, endPage); }
    ...
    const char *SaveDB(bool *success, bool istempfile = false, int page = -1)
    {
        ...
        {  // limit destructors
            ...
            {
                if (!istempfile)
                    wxMessageBox(L"Error writing TreeSheets file! (try saving under new filename)", sfn.wx_str(), wxOK,
                                 sys->frame);
        ...
        if (!scaledviewingmode)
        {
            ...
            if ((layoutys > canvash || layoutxs > canvasw) && s.g)
            {
                ...
                if (r.y < originy || r.y + r.height > maxy || r.x < originx || r.x + r.width > maxx)
                {
                    ...
                    sw->SetScrollbars(
                        1, 1, layoutxs, layoutys,
                        r.width > canvasw || r.x < originx ? r.x : r.x + r.width > maxx ? r.x + r.width - canvasw
                                                                                        : curx,
                        r.height > canvash || r.y < originy ? r.y : r.y + r.height > maxy ? r.y + r.height - canvash
                                                                                          : cury,
                        true);
        ...
        for (Cell *p = curdrawroot->parent; p; p = p->parent)
            if (p->text.t.Len())
            {
                ...
                if ((int)s.Len() > sys->defaultmaxcolwidth)
                    s = s.Left(sys->defaultmaxcolwidth) + L"...";  // should take the width of these into account for
                                                                   // layoutys, but really, the worst that can happen on
                                                                   // a thin window is that its rendering gets cut off
    ...
    bool PickFont(wxDC &dc, int depth, int relsize, int stylebits)
    {
        ...
        if (textsize != lasttextsize || stylebits != laststylebits)
        {
            dc.SetFont(wxFont(textsize - (while_printing || scaledviewingmode),
                              ...
                              (stylebits & STYLE_UNDERLINE) != 0, stylebits & STYLE_FIXED ? L"" : sys->defaultfont));
    ...
    bool CheckForChanges()
    {
        if (modified)
        {
            ThreeChoiceDialog tcd(sys->frame, filename, L"Changes have been made, are you sure you wish to continue?",
    ...
    const char *ExportFile(const wxString &fn, int k)
    {
        if (k == A_EXPCSV)
        {
            ...
            if (maxdepth > 1)
                return "cannot export grid that is not flat (zoom the view to the desired grid, and/or use Flatten)";
        }
    ...
    const char *Save(bool saveas, bool *success = NULL)
    {
        ...
        wxString fn = ::wxFileSelector(L"Choose TreeSheets file to save:", L"", L"", L"cts", L"*.cts",
    ...
    void AutoSave(bool minimized, int page)
    {
        if (tmpsavesuccess && !filename.empty() && lastmodsinceautosave &&
            (lastmodsinceautosave + 60 < wxGetLocalTime() || lastsave + 300 < wxGetLocalTime() || minimized))
        {
    ...
    const char *Key(wxDC &dc, wxChar uk, int k, bool alt, bool ctrl, bool shift, bool &unprocessed)
    {
        ...
        if (uk == WXK_NONE || (k < ' ' && k))
        {
            switch (k)
            {
                ...
                #ifdef __WXGTK__
                // should not be needed... on Windows / OS X they arrive as menu event and never arrive here, on Linux
                ...
                case WXK_TAB:
                    if (ctrl && !shift)
                    {
                        // WXK_CONTROL_I (italics) arrives as the same keycode as WXK_TAB + ctrl on Linux??
                        // They're both keycode 9 in defs.h
                        // We ignore it here, such that CTRL+I works, but it means only SHIFT+CTRL+TAB works on Linux as
                        // a way to switch tabs.
                        // Also, even though we ignore CTRL+TAB, and it is not assigned in the menus, it still has the
                        // effect of de-selecting
                        // the current tab (requires a click to re-activate). FIXME??
                        break;
                    }
                    return Action(dc, shift ? (ctrl ? A_PREVFILE : A_PREV) : (ctrl ? A_NEXTFILE : A_NEXT));
                #endif
            }
        }
        else if (uk >= ' ')
        {
            ...
            c->AddUndo(
                this);  // FIXME: not needed for all keystrokes, or at least, merge all keystroke undos within same cell
    ...
    const char *Action(wxDC &dc, int k)
    {
        ...
        switch (k)
        {
            ...
            case A_OPEN:
            {
                wxString fn = ::wxFileSelector(L"Please select a TreeSheets file to load:", L"", L"", L"cts", L"*.cts",
            ...
            case A_NEW:
            {
                int size = ::wxGetNumberFromUser(L"What size grid would you like to start with?", L"size:",
                                                 L"New Sheet", 10, 1, 25, sys->frame);
            ...
            {
                wxFontData fdat;
                fdat.SetInitialFont(wxFont(g_deftextsize, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL,
                                           false, sys->defaultfont));
            ...
            case A_PRINTSCALE:
            {
                printscale = ::wxGetNumberFromUser(L"How many pixels wide should a page be? (0 for auto fit)",
                                                   L"scale:", L"Set Print Scale", 0, 0, 5000, sys->frame);
                return NULL;
            }
            ...
            case A_PREVIEW:
            {
                ...
                wxPreviewFrame *pframe =
                    new wxPreviewFrame(preview, sys->frame, L"Print Preview", wxPoint(100, 100), wxSize(600, 650));
            ...
            case A_DEFBGCOL:
            {
                ...
                if (c != (uint)-1) {
                    ...
                    loopallcells(lc)
                    {
                        if (lc->cellcolor == oldbg && (!lc->parent || lc->parent->cellcolor == c)) lc->cellcolor = c;
                    }
                    Refresh();
                }
                return NULL;
            ...
            case A_SCALED:
                ...
                return scaledviewingmode
                           ? "now viewing TreeSheet to fit to the screen exactly, press F12 to return to normal"
                           : "1:1 scale restored";
        ...
        switch (k)
        {
            ...
            case A_DELETE:
                ...
                {
                    if (selected.xs)
                        DelRowCol(selected.y, selected.g->ys, selected.g->ys, 0, -1, selected.y, 0, -1);
                    else
                        DelRowCol(selected.x, selected.g->xs, selected.g->xs, 0, selected.x, -1, -1, 0);
                }
                else if (c && selected.TextEdit())
                {
            ...
            {
                if (!(c = selected.ThinExpand(this))) return OneCell();
                wxString fn = ::wxFileSelector(L"Please select an image file:", L"", L"", L"", L"*.*",
                                               wxFD_OPEN | wxFD_FILE_MUST_EXIST | wxFD_CHANGE_DIR);
                c->AddUndo(this);
            ...
            case A_MINISIZE:
            {
                ...
                loopv(i, outer)
                {
                    ...
                    if (o->grid)
                    {
                        loopcellsin(o, c) if (_i) { c->text.relsize = g_deftextsize - g_mintextsize() - c->Depth(); }
                    }
                }
                outer.setsize_nd(0);
                selected.g->cell->ResetChildren();
        ...
        if (c || (!c && selected.IsAll()))
        {
            ...
            switch (k)
            {
                ...
                case A_HIFY:
                    ...
                    if (!ac->grid->IsTable())
                        return "selected grid is not a table: cells must not already have sub-grids";
                    ac->AddUndo(this);
        ...
        switch (k)
        {
            ...
            case A_IMAGESC:
            {
                ...
                long v = wxGetNumberFromUser(L"Please enter the percentage you want the image scaled by:", L"%",
        ...
        switch (dataobjc->GetReceivedFormat().GetType())
        {
            ...
            default:  // several text formats
                if (dataobjt->GetText() != wxEmptyString) {
                    ...
                    /*
                    #ifdef __WXMAC__
                    // bug in wxmac where it takes UTF-8 from the system, and interprets it as UTF-16, then converts it
                    to UTF-8. Here we do the opposite, sortof
    ...
    const char *Sort(bool descending)
    {
        if (selected.xs != 1 && selected.ys <= 1)
            return "can't sort: make a 1xN selection to indicate what column to sort on, and what rows to affect";
        selected.g->cell->AddUndo(this);
    ...
    bool LastUndoSameCell(Cell *c)
    {
        return undolist.size() && !c->grid && undolist.size() != undolistsizeatfullsave &&
               ...
               (!c->text.t.EndsWith(" ") || c->text.t.Len() != selected.cursor);  // hacky way to detect word boundaries
                                                                                  // to stop coalescing, but works, and
                                                                                  // not a big deal if selected is not
                                                                                  // actually related to this cell
    }
    ...
    static int _timesort(const Cell **a, const Cell **b) { return ((*a)->text.lastedit < (*b)->text.lastedit) * 2 - 1; }
