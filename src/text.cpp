...
Text::Text() : cell(NULL), t(wxEmptyString), relsize(0), stylebits(0), extent(0), image(NULL), filtered(false) { WasEdited(); }

double Text::GetNum() { wxChar *end; double r = wcstod(t.c_str(), &end); /*HUGE_VAL*/ return r; }
...
wxString Text::htmlify(wxString &str)
{
    wxString r;
    loop(i, str.Len())
    {
...
void Text::RelSize(int dir, int zoomdepth) { relsize = max(min(relsize+dir, g_deftextsize-g_mintextsize()+zoomdepth), g_deftextsize-g_maxtextsize()-zoomdepth); }
...
bool Text::IsInSearch() { return sys->searchstring.Len() && t.Lower().Find(sys->searchstring)>=0; }

int Text::Render(Document *doc, int bx, int by, int depth, wxDC &dc, int &leftoffset, int maxcolwidth)
{
    int ixs = 0, iys = 0;
...
void Text::DrawCursor(Document *doc, wxDC &dc, Selection &s, bool full, uint color, bool cursoronly, int maxcolwidth)
{
    int ixs = 0, iys = 0;
    if(!cell->tiny) sys->ImageSize(image, ixs, iys);
    if(ixs) ixs += 2;
...
void Text::Delete(Selection &s)             { ifrangeselremove else if(s.cursor<(int)t.Len()) { t.Remove(s.cursor, 1); }; }
void Text::Backspace(Selection &s)          { ifrangeselremove else if(s.cursor>0)            { t.Remove(--s.cursor, 1); --s.cursorend; }; }
void Text::Key(int k, Selection &s)         { ifrangeselremove; setrelsize; t.insert(s.cursor++, 1, k); s.cursorend = s.cursor; }
...
void Text::Clear(Document *doc, Selection &s)              { t.Clear(); s.EnterEdit(doc); }
...
void Text::Save(wxDataOutputStream &dos)
{
