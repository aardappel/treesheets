...
struct Text
{
    ...
    Text() : cell(NULL), t(wxEmptyString), relsize(0), stylebits(0), extent(0), image(NULL), filtered(false)
    {
        WasEdited();
    }
    ...
    wxString GetLinePart(int &i, int p, int l)
    {
        ...
        while (i < l && t[i] == L' ')  // gobble spaces, but do not copy them
        {
            i++;
            if (i == l)
                p = i;  // happens with a space at the last line, user is most likely about to type another word, so
                        // need to show space. Alternatively could check if the cursor is actually on this spot. Simply
                        // showing a blank new line would not be a good idea, unless the cursor is here for sure, and
    ...
    wxString GetLine(int &i, int maxcolwidth)
    {
        ...
        // for(int p = i+maxcolwidth; p<l;  p++) if(!IsWord(t[p])) return GetLinePart(i, p, l);  // we arrive here only
            ...
            for (int l = 0;; l++)
            {
                ...
                if (s.cursor != s.cursorend)
                {
                    if (s.cursor <= end && s.cursorend >= start && !cursoronly)
                    {
                        ...
                        if (x1 != x2)
                            DrawRectangle(dc, color, cell->GetX(doc) + x1 + 2 + ixs + g_margin_extra,
                                          cell->GetY(doc) + l * h + 1 + cell->ycenteroff + g_margin_extra, x2 - x1,
                                          h - 1
                                          #ifdef SIMPLERENDER
                                          ,
                                          true
                ...
                else if (s.cursor >= start && s.cursor <= end)
                {
                    ...
                    if (doc->blink)
                        ...
                        DrawRectangle(dc, color, cell->GetX(doc) + x + 1 + ixs + g_margin_extra,
                                      cell->GetY(doc) + l * h + 1 + cell->ycenteroff + g_margin_extra, 2, h - 2);
                        #else
    ...
    #define ifrangeselremove                            \
        if (WasEdited(), s.cursor != s.cursorend)       \
        {                                               \
    ...
    {
        ifrangeselremove else if (s.cursor < (int)t.Len()) { t.Remove(s.cursor, 1); };
    }
    void Backspace(Selection &s)
    {
        ifrangeselremove else if (s.cursor > 0)
        {
            t.Remove(--s.cursor, 1);
            --s.cursorend;
        };
