...
struct System
{
    ...
    System(bool portable)
        : cfg(portable ? (wxConfigBase *)new wxFileConfig(L"", wxT(""), wxGetCwd() + wxT("/TreeSheets.ini"), wxT(""), 0)
    ...
    const char *LoadDB(const wxString &filename, bool frominit = false, bool fromreload = false)
    {
        ...
        if (!fromreload)
        {
            ...
            {
                if (::wxMessageBox(L"A temporary autosave file exists, would you like to load it instead?",
            ...
            for (;;)
            {
                ...
                switch (*buf)
                {
                    ...
                    case 'D':
                    {
                        ...
                        doc->sw->Status(wxString::Format(L"loaded %s (%d cells, %d characters)", filename.c_str(),
                                                         numcells, textbytes)
                                            .c_str());
        ...
        if (anyimagesfailed)
            wxMessageBox(L"PNG decode failed on some images in this document\nThey have been replaced by red squares.",
    ...
    void FillXML(Cell *c, wxXmlNode *n, bool attributestoo)
    {
        const wxArrayString &as =
            wxStringTokenize(n->GetType() == wxXML_ELEMENT_NODE ? n->GetNodeContent() : n->GetContent());
        loop(i, as.GetCount())
        {
