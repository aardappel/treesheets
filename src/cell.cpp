...
Cell::Cell(Cell *_p, Cell *_clonefrom, int _ct, Grid *_g)
...
bool Cell::HasText()      { return !text.t.empty(); }
bool Cell::HasTextSize()  { return HasText() || text.relsize; }
bool Cell::HasTextState() { return HasTextSize() || text.image; }
bool Cell::HasHeader()    { return HasText() || text.image; }
bool Cell::HasContent()   { return HasHeader() || grid; }

bool Cell::GridShown(Document *doc) { return grid && (!grid->folded || this==doc->curdrawroot); }
...
void Cell::Render(Document *doc, int bx, int by, wxDC &dc, int depth, int ml, int mr, int mt, int mb, int maxcolwidth, int cell_margin)
{
    switch(celltype)
    {
    ...
    if(drawstyle!=DS_GRID && HasContent() && !tiny)
    {
        ...
        {
            if(actualcellcolor==doc->Background()) actualcellcolor = 0xDCDCDC;
        }
        */
...
void Cell::CloneStyleFrom(Cell *o)
{
...
bool Cell::IsInside(int x, int y) { return x>=0 && y>=0 && x<sx && y<sy; }

int Cell::GetX(Document *doc) { return ox+(parent ? parent->GetX(doc) : doc->hierarchysize); }
int Cell::GetY(Document *doc) { return oy+(parent ? parent->GetY(doc) : doc->hierarchysize); }

int Cell::Depth() { return parent ? parent->Depth()+1 : 0; }
Cell *Cell::Parent(int i) { return i ? parent->Parent(i-1) : this; }
Cell *Cell::SetParent(Cell *g) { parent = g; return this; }
...
void Cell::ResetChildren() { Reset(); if(grid) grid->ResetChildren(); }
...
void Cell::Save(wxDataOutputStream &dos)
{
    dos.Write8(celltype);
...
Cell *Cell::FindLink(Selection &s, Cell *link, Cell *best, bool &lastthis, bool &stylematch)
{
...
Cell *Cell::FindExact(wxString &s)
{
...
int Cell::ColWidth()
{
    return parent ? parent->grid->colwidths[parent->grid->FindCell(this).x] : sys->defaultmaxcolwidth;
}
