struct TSFrame : wxFrame {
    ...
    TSFrame(wxString exename, TSApp *_app)
        ...
        loop(twoeditmenus, 2) {
            ...
            MyAppend(temenu, A_ENTERCELL_JUMPTOEND,
                     _(L"...and jump to the end of the text") + "\tF2");
            MyAppend(
                temenu, A_ENTERCELL_JUMPTOSTART,
                _(L"...and progress to the first cell in the new row") + "\t" ALTORCTRL "+ENTER");
            MyAppend(temenu, A_PROGRESSCELL,
                     _(L"...and progress to the next cell on the right") + "\t" CTRLORALT "+ENTER");
            ...
            imgmenu->AppendSeparator();
            MyAppend(imgmenu, A_LASTIMAGE, _(L"Insert last image") + "\tSHIFT+ALT+i",
                     (L"Insert the last image that has been inserted before in TreeSheets."));
    ...
    void CycleTabs(size_t offset = 1) {
        auto numtabs = notebook->GetPageCount();
